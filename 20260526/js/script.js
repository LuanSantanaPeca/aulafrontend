...
let valorTotal=0;
function gerenciarCompras(produtoId){
    ...
    if(produto.classList.contains('selected')){
        ...
        switch(produto.dataset.descr){
            case "Notebook":
                valorTotal -= notebookVal;
                notebookVal = 0;
                notebookItem?.remove();
                notebookItem = null;
                break;
            case "Celular":
                valorTotal -= celularVal;
                celularVal = 0;
                celularItem?.remove();
                celularItem = null;
                break;
            case "Tablet":
                valorTotal -= tabletVal;
                tabletVal = 0;
                tabletItem?.remove();
                tabletItem = null;
                break;
        ...
        let quant = prompt("Informe a quantidade desejada");
        quant = quant.replace(/\D+/g, "");
        if(quant == ""){
            alert("A quantidade deve ser um número");
            return;
        }
        const preco = Number(quant) * Number(produto.dataset.preco);
        ...
        switch(produto.dataset.descr){
            case "Notebook":
                notebookVal = preco;
                notebookItem = novoItem;
                break;
            case "Celular":
                celularVal = preco;
                celularItem = novoItem;
                break;
            case "Tablet":
                tabletVal = preco;
                tabletItem = novoItem;
